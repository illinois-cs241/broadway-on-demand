...
class Visibility:
    ...
    VISIBLE = "visible"
    VISIBLE_FROM_START = (
        "visible_from_start"  # visible from start date of the assignment
    )
...
def overwrite_student_roster(cid, students):
    ...
    for student in students:
        student_dict = {"netid": student[0], "uin": student[1], "name": student[2]}
...
def get_assignments_for_course(cid, visible_only=False):
    if visible_only:
        ...
        # if always visible
        visible_always = {"visibility": Visibility.VISIBLE}
        return list(
            mongo.db.assignments.find(
                {"course_id": cid, "$or": [visible_from_start_date, visible_always]}
            )
        )
    else:
...
def get_scheduled_runs(cid, aid):
    ...
    :return: a list of objects, each containing a list of runs for a single student.
    """
    return mongo.db.scheduled_runs.find({"course_id": cid, "assignment_id": aid}).sort(
        "run_time", DESCENDING
    )
...
def get_extensions(cid, aid, netid=None):
    if netid is None:
        return mongo.db.extensions.find({"course_id": cid, "assignment_id": aid}).sort(
            "netid", ASCENDING
        )
    else:
        return mongo.db.extensions.find(
            {"course_id": cid, "assignment_id": aid, "netid": netid}
        )
    ...
    return mongo.db.extensions.insert_one(
        {
            ...
            "start": start,
            "end": end,
            "run_id": None or str(scheduled_run_id),
        }
    )
...
def delete_extension(cid, aid, extension_id):
    ...
    if run_id:
        try:
            ...
            print(
                f"Failed to delete scheduled run for extension id {extension_id} run id {run_id}: ",
                str(e),
                flush=True,
            )
            pass
